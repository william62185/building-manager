...
class ApartmentsListView(tk.Frame):
    ...
    def _create_search_panel(self, parent):
        ...
        fg = theme.get("text_primary", "#1f2937")
        panel_bg = "#f3e8ff"
        panel = tk.Frame(parent, bg=panel_bg, width=250)
        panel.pack_propagate(False)
        tk.Label(panel, text="🔍 Filtros", font=("Segoe UI", 12, "bold"), bg=panel_bg, fg=fg).pack(pady=Spacing.MD)
        tk.Label(panel, text="Búsqueda General:", font=("Segoe UI", 10), bg=panel_bg, fg=fg).pack(anchor="w", padx=Spacing.MD)
        self.search_var = tk.StringVar()
        self.search_var.trace_add("write", lambda *_: self._apply_filters())
        self.search_entry = ttk.Entry(panel, textvariable=self.search_var, font=("Segoe UI", 10))
        self.search_entry.pack(fill="x", padx=Spacing.MD, pady=(0, Spacing.LG))
        tk.Label(panel, text="Edificio:", font=("Segoe UI", 10), bg=panel_bg, fg=fg).pack(anchor="w", padx=Spacing.MD)
        ...
        tk.Label(panel, text="Estado:", font=("Segoe UI", 10), bg=panel_bg, fg=fg).pack(anchor="w", padx=Spacing.MD)
        ...
        ModernButton(panel, text="Limpiar Filtros", style="purple", command=self._clear_filters).pack(pady=Spacing.LG)
        return panel

    def _create_list_panel(self, parent):
        cb = self._content_bg
        panel = tk.Frame(parent, bg=cb)
        self.canvas = tk.Canvas(panel, bg=cb, highlightthickness=0)
        scrollbar = ttk.Scrollbar(panel, orient="vertical", command=self.canvas.yview)
        self.list_panel = tk.Frame(self.canvas, bg=cb)
        
        def _update_scroll_region(e=None):
            if self.canvas and self.list_panel.winfo_exists():
                self.canvas.configure(scrollregion=self.canvas.bbox("all"))
        self.list_panel.bind("<Configure>", _update_scroll_region)
        ...
        # Scroll solo cuando el ratón está sobre el panel (canvas + lista); al salir se desactiva
        panel.bind("<Enter>", self._bind_mousewheel)
        panel.bind("<Leave>", self._unbind_mousewheel)
    ...
    def _focus_search_entry(self):
        """Coloca el foco en el cuadro de búsqueda al abrir la vista de listado de apartamentos."""
    ...
    def _display_apartments(self, apartments: List[Dict[str, Any]]):
        ...
        if not apartments:
            theme = theme_manager.themes[theme_manager.current_theme]
            fg = theme.get("text_primary", "#1f2937")
            tk.Label(self.list_panel, text="No se encontraron apartamentos.", font=("Segoe UI", 10), bg=self._content_bg, fg=fg).pack(pady=Spacing.XL)
            return
        ...
        if self.canvas and self.canvas.winfo_exists():
            self.list_panel.update_idletasks()
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))
    ...
    def _create_apartment_card(self, parent, apt: Dict[str, Any]):
        ...
        card.pack(fill="x", pady=(0, Spacing.SM), padx=Spacing.SM)
        content = tk.Frame(card, bg=card_bg)
        content.pack(fill="x", padx=Spacing.SM, pady=Spacing.SM)
        info_frame = tk.Frame(content, bg=card_bg)
        ...
        tk.Label(info_frame, text=title_text, font=("Segoe UI", 12, "bold"), bg=card_bg, fg="#1f2937").pack(side="left")
        status_colors = {"Disponible": "green", "Ocupado": "orange", "En Mantenimiento": "blue"}
        status = apt.get("status", "N/A")
        tk.Label(info_frame, text=f"● {status}", fg=status_colors.get(status, "black"), font=("Segoe UI", 9, "bold"), bg=card_bg).pack(side="right")
        ...
        tk.Label(details_frame, text=details_text, font=("Segoe UI", 9), bg=card_bg, fg="#374151").pack(anchor="w")
        if apt.get('status') == 'Ocupado':
            ...
            tk.Label(details_frame, text=f"Inquilino: {tenant_name}", font=("Segoe UI", 9, "italic"), bg=card_bg, fg="#374151").pack(anchor="w", pady=(1, 0))
        actions_frame = tk.Frame(content, bg=card_bg)
        actions_frame.pack(fill="x", pady=(Spacing.SM, 0))
        edit_button = ModernButton(actions_frame, text="Editar", icon="✏️", style="purple", small=True, command=lambda a=apt: self.on_edit(a, self.get_filter_state()))
    ...
    def _on_destroy(self, event):
        """Cleanup: desenlazar scroll al destruir la vista."""
    ...
    def _bind_mousewheel(self, event):
        """Activa scroll solo cuando el ratón entra en el área del canvas."""
        try:
            self.unbind_all("<MouseWheel>")
        except tk.TclError:
            pass
    ...
    def _unbind_mousewheel(self, event):
        """Desactiva scroll al salir del área del canvas (evita que se quede pegado)."""
        try:
            self.unbind_all("<MouseWheel>")
        except tk.TclError:
            pass
