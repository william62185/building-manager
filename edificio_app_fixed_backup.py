...
class TenantModule:
    ...
    def setup_ui(self, parent):
        """Configura la interfaz de la pestaña de inquilinos"""
        # Frame principal
        self.frame = ttk.Frame(parent)
        parent.add(self.frame, text=" 👥 Inquilinos ")

        # Dashboard
        dashboard_frame = ttk.Frame(self.frame, style='Main.TFrame')
        dashboard_frame.pack(fill="x", padx=10, pady=5)

        # Título del Dashboard con estilo mejorado
        ...
        stats_container = ttk.Frame(dashboard_frame, style='Main.TFrame')
        stats_container.pack(fill="x", pady=10)
        self.create_stat_cards(stats_container)

        # Botones de acción principales
        action_frame = ttk.Frame(dashboard_frame, style='Main.TFrame')
        action_frame.pack(fill="x", pady=10)

        # Botón para ver listado completo
        btn_listado = create_styled_button(
            action_frame,
            "📋 Ver Listado Completo",
            self.abrir_ventana_listado_inquilinos
        )
        btn_listado.pack(side="left", padx=5)

        # Botón para agregar inquilino
        btn_agregar = create_styled_button(
            action_frame,
            "➕ Agregar Inquilino",
        ...
        btn_reporte = create_styled_button(
            action_frame,
            "📊 Generar Reporte",
            self.generar_reporte_inquilinos
        )
        btn_reporte.pack(side="left", padx=5)

        # Botón para exportar datos
        btn_exportar = create_styled_button(
            action_frame,
            "📤 Exportar Datos",
            self.exportar_datos_inquilinos
        )
        btn_exportar.pack(side="left", padx=5)

        # Botón para importar datos
        btn_importar = create_styled_button(
            action_frame,
            "📥 Importar Datos",
            self.importar_datos_inquilinos
        )
...
class ApartmentManager:
    def __init__(self, root):
        ...
        # Inicializar módulos
        self.dashboard_module = DashboardModule(self)
    ...
    def setup_ui(self):
        ...
        # Notebook (pestañas) con estilo mejorado
        self.notebook = ttk.Notebook(main_frame)
        self.notebook.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Configurar pestañas con texto descriptivo
        self.dashboard_module.setup_ui(self.notebook)
        self.tenant_module.setup_ui(self.notebook)
        self.payment_module.setup_ui(self.notebook)
        self.expense_module.setup_ui(self.notebook)
        self.report_module.setup_ui(self.notebook)
        self.backup_module.setup_ui(self.notebook)
...
class DashboardModule:
    ...
    def setup_ui(self, parent):
        """Configura la interfaz del dashboard principal"""
        self.frame = ttk.Frame(parent)
        ...
        # Canvas y scrollbar
        canvas = tk.Canvas(main_frame, highlightthickness=0, bg="#ffffff")
        scrollbar = ttk.Scrollbar(main_frame, orient="vertical", command=canvas.yview)
        ...
        title_frame = ttk.Frame(self.scroll_frame, style="Main.TFrame")
        ...
        subtitle = ttk.Label(title_frame,
                           text="Bienvenido al Sistema de Administración de Edificios",
                           font=("Segoe UI", 12),
                           foreground="#5f6368")
        subtitle.pack()
        
        # === MÉTRICAS ===
        metrics_frame = ttk.Frame(self.scroll_frame, style="Main.TFrame")
        metrics_frame.pack(fill="x", padx=20, pady=(0, 30))
        
        # Configurar grid para métricas
        ...
        self.inquilinos_metric = self.create_metric(metrics_frame,
            "👥", "Inquilinos Activos", "0", "inquilinos")
        self.inquilinos_metric.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        
        # Métrica de Ingresos
        self.ingresos_metric = self.create_metric(metrics_frame,
            "💰", "Ingresos del Mes", "$0", "ingresos")
        self.ingresos_metric.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        
        # Métrica de Gastos
        self.gastos_metric = self.create_metric(metrics_frame,
            "💳", "Gastos del Mes", "$0", "gastos")
        self.gastos_metric.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
        
        # === CARDS ===
        cards_frame = ttk.Frame(self.scroll_frame, style="Main.TFrame")
        cards_frame.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        # Configurar grid de 2x2 para las cards
        cards_frame.grid_columnconfigure(0, weight=1, uniform="column")
        cards_frame.grid_columnconfigure(1, weight=1, uniform="column")
        cards_frame.grid_rowconfigure(0, weight=1, uniform="row")
        cards_frame.grid_rowconfigure(1, weight=1, uniform="row")
        
        # === CARD DE INQUILINOS ===
        inquilinos_card = self.create_card(cards_frame,
            "👥", "Inquilinos", "Ver detalles de inquilinos, agregar nuevos, editar existentes y más.",
            lambda: self.manager.notebook.select(1))
        inquilinos_card.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        
        # === CARD DE PAGOS ===
        pagos_card = self.create_card(cards_frame,
            "💰", "Pagos", "Registrar pagos, generar recibos y ver historial de transacciones.",
            lambda: self.manager.notebook.select(2))
        pagos_card.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        
        # === CARD DE GASTOS ===
        gastos_card = self.create_card(cards_frame,
            "💳", "Gastos", "Administrar gastos del edificio y ver reportes detallados.",
            lambda: self.manager.notebook.select(3))
        gastos_card.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")
        
        # === CARD DE REPORTES ===
        reportes_card = self.create_card(cards_frame,
            "📊", "Reportes", "Generar reportes detallados y ver estadísticas importantes.",
            lambda: self.manager.notebook.select(4))
        reportes_card.grid(row=1, column=1, padx=10, pady=10, sticky="nsew")
    ...
    def create_card(self, parent, icon_text, title, description, command):
        ...
        # Frame clickeable que envuelve todo
        clickable_frame = tk.Frame(parent, cursor="hand2", bg="#ffffff")
        clickable_frame.bind("<Button-1>", lambda e: command())
        
        # Card principal
        card = ttk.Frame(clickable_frame, style="Card.TFrame")
    ...
    def actualizar_metricas(self):
        ...
        try:
            ...
            self.inquilinos_value_label.configure(text=str(total_activos))
            ...
            self.ingresos_value_label.configure(text=f"${ingresos_mes:,.2f}")
            ...
            self.gastos_value_label.configure(text=f"${gastos_mes:,.2f}")
    ...
    def create_metric(self, parent, icon, title, value, metric_type):
        """Crea un widget de métrica con estilo Material Design"""
        metric = ttk.Frame(parent, style="Card.TFrame")
        
        # Contenido de la métrica
        content = ttk.Frame(metric)
        ...
        # Guardar referencia a los labels según el tipo de métrica
        if metric_type == "inquilinos":
            self.inquilinos_value_label = value_label
        elif metric_type == "ingresos":
            self.ingresos_value_label = value_label
        elif metric_type == "gastos":
            self.gastos_value_label = value_label
        
        def on_enter(e):
            metric.configure(style="CardHover.TFrame")
            
        def on_leave(e):
            metric.configure(style="Card.TFrame")
            
        metric.bind("<Enter>", on_enter)
        metric.bind("<Leave>", on_leave)
    ...
    def create_card(self, parent, icon_text, title, description, command):
        ...
        for widget in [card, content, header, icon, title_label, desc_label, btn]:
            widget.bind("<Enter>", on_enter)
            widget.bind("<Leave>", on_leave)
