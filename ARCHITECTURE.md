...
- **Módulo Pagos:** Al hacer clic en "Pagos" en el sidebar se abre directamente la vista de listado/edición (`EditDeletePaymentsView`: búsqueda por inquilino, dos cards "Registrar nuevo pago" y "Reportes" del mismo tamaño, tabla de pagos). No existe la pantalla intermedia "¿Qué deseas hacer?" con tres cards. En esta vista solo se muestra el botón "Dashboard" (no "Volver"). El card "Registrar nuevo pago" abre un modal compacto (`PaymentModal` en modo `compact=True`) que toma el inquilino del filtro de búsqueda; el modal muestra "Cambiar inquilino" (autocompletado) arriba, luego tres líneas en negrita (Inquilino, Identificación, Apartamento), fecha con mini selector, monto, método, observaciones, y botón "Registrar Pago"; se cierra con Escape (binding global `bind_all`) sin necesidad de foco; al guardar o eliminar un pago solo se reproduce sonido de confirmación (sin mensaje "Éxito"). "Volver" desde Reportes de pagos vuelve al listado (`_show_edit_delete_payments`).
...
- **Módulo Pagos (v1.0.1+):** Entrada directa al listado (`PaymentsView.__init__` llama `_show_edit_delete_payments()`). Vista listado: cards igual tamaño (grid 50/50), sin título "Editar/Eliminar Pagos"; botón "Volver" oculto (`show_back_button=False`). Modal "Registrar Pago" compacto: búsqueda primero, 3 líneas inquilino en negrita, DatePickerWidget, botón "Limpiar" para cambiar inquilino; placeholder del autocomplete se borra al escribir (KeyPress); cierre con Escape vía `bind_all`; confirmaciones solo con sonido (`winsound.MessageBeep(MB_ICONASTERISK)`). Eliminación de pago: mismo sonido, sin mensaje "Eliminado".
- **Combobox (ttk.Combobox) — UX unificada:** Para que el listado se abra al hacer clic en el campo de texto (no solo en la flecha), y que la flecha siga abriendo de forma nativa, se usa el helper `bind_combobox_dropdown_on_click(combobox)` en `ui/components/modern_widgets.py`. **Aplicar a todos los combobox** al crearlos (Búsqueda Avanzada inquilinos, método de pago en formularios y modales, etc.). En desarrollos futuros, cualquier nuevo `ttk.Combobox` debe llamar a este helper tras su creación.
